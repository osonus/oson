 ...
 */
public final class JsonAdapterAnnotationOnClassesTest extends TestCaseBase {

  public void testJsonAdapterInvoked() {
    Gson gson = new Gson();
    String json = oson.ser(A.class, (Object p) -> "jsonAdapter").toJson(new A("bar"));
    assertEquals("\"jsonAdapter\"", json);

   // Also invoke the JsonAdapter javadoc sample
    json = oson.ser(User.class, (Object p) -> {
    	User user = (User)p;
    	JSONObject node = new JSONObject();
    	node.put("name", user.firstName + " " + user.lastName);
    	return node.toString();
    }).toJson(new User("Inderjeet", "Singh"));
    assertEquals("{\"name\":\"Inderjeet Singh\"}", json);
    
    User user = oson.des(User.class, (DataMapper classData) -> {
    	Map map = (Map)classData.getValueObject();
    	String data[] = map.get("name").toString().split(" ");
    	return new User(data[0], data[1]);
    }).fromJson("{'name':'Joel Leitch'}", User.class);
    
    assertEquals("Joel", user.firstName);
    assertEquals("Leitch", user.lastName);

    json = oson.ser(Foo.class, (Object p) -> {
    	return StringUtil.doublequote(p.toString().toLowerCase(Locale.US));
    }).toJson(Foo.BAR);
    
    assertEquals("\"bar\"", json);
    
    
    Foo baz = oson.ser(Foo.class, (Object p) -> {
    	return Foo.valueOf(StringUtil.unquote(p.toString().toUpperCase(Locale.US), false));
    }).fromJson("\"baz\"", Foo.class);
  ...
  public void testJsonAdapterFactoryInvoked() {
    ...
    String json = oson.ser(C.class, (Object p) -> "jsonAdapterFactory").toJson(new C("bar"));
    assertEquals("\"jsonAdapterFactory\"", json);
    
    
    C c = oson.des(C.class, (Object p) -> new C("jsonAdapterFactory")).fromJson("\"bar\"", C.class);
  ...
  public void testRegisteredAdapterOverridesJsonAdapter() {
    ...
    String json = oson.ser(A.class, (Object p) -> "registeredAdapter").toJson(new A("abcd"));
  ...
  public void testRegisteredSerializerOverridesJsonAdapter() {
    ...
    String json = oson.ser(A.class, (Object p) -> "registeredSerializer").toJson(new A("abcd"));
    assertEquals("\"registeredSerializer\"", json);
    
    A target = oson.des(A.class, (Object p) -> new A("jsonAdapter")).fromJson("abcd", A.class);
  ...
  public void testRegisteredDeserializerOverridesJsonAdapter() {
    ...
    String json = oson.ser(A.class, (Object p) -> "jsonAdapter").toJson(new A("abcd"));
    
    assertEquals("\"jsonAdapter\"", json);
    
    A target = oson.des(A.class, (Object p) -> new A("registeredDeserializer")).fromJson("abcd", A.class);
  ...
  public void testNullSafeObjectFromJson() {
    Gson gson = new Gson();
    NullableClass fromJson = oson.fromJson("null", NullableClass.class);
    assertNull(fromJson);
  }
