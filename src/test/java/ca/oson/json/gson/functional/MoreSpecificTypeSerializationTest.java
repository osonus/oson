...
public class MoreSpecificTypeSerializationTest extends TestCaseBase {
  private Gson gson;
  ...
  public void testSubclassFields() {
    ...
    String json = oson.toJson(target);//.clearAll()
    //{"b":{"b":1}}  {"b":{"s":2,"b":1}}
...
//    JsonObject json = oson.toJsonTree(target).getAsJsonObject().get("map").getAsJsonObject();
...
//  }
  ...
  public void testListOfParameterizedSubclassFields() {
    ...
    //{"collection":[{"t":"one"},{"t":"two"}]}
    //{"collection":[{"t":"one"},{"s":"three","t":"two"}],"map":null}
    String json = oson.toJson(target);//.clearAll().setDefaultType(JSON_INCLUDE.NON_NULL)
    assertTrue(json, json.contains("{\"t\":\"one\"}"));
    assertTrue(json, json.contains("\"s\":"));
  }
//
//  /**
