...
public final class TypeAdapterPrecedenceTest extends TestCaseBase {
  public void testNonstreamingFollowedByNonstreaming() {
    Gson gson = new GsonBuilder()
        ...
        .create();
    assertEquals("\"foo via serializer 2\"", oson.ser(Foo.class, (Object p) -> {
    	Foo foo = (Foo)p;
    	return new JsonPrimitive(foo.name + " via serializer 2");
    }).toJson(new Foo("foo")));
    assertEquals("foo via deserializer 2", oson.des(Foo.class, (DataMapper p) -> {
    	String value = (String) p.getValueObject();
    	return new Foo (value + " via deserializer 2");
    }).fromJson("foo", Foo.class).name);
  }
  ...
  public void testStreamingFollowedByStreaming() {
    Gson gson = new GsonBuilder()
        ...
        .create();
    assertEquals("\"foo via type adapter 2\"", oson.ser(Foo.class, (Object p) -> {
    	Foo foo = (Foo)p;
    	return new JsonPrimitive(foo.name + " via type adapter 2");
    }).toJson(new Foo("foo")));
    assertEquals("foo via type adapter 2", oson.des(Foo.class, (DataMapper p) -> {
    	String value = (String) p.getValueObject();
    	return new Foo (value + " via type adapter 2");
    }).fromJson("foo", Foo.class).name);
  }
  ...
  public void testSerializeNonstreamingTypeAdapterFollowedByStreamingTypeAdapter() {
    Gson gson = new GsonBuilder()
        ...
        .create();
    assertEquals("\"foo via type adapter\"", oson.ser(Foo.class, (Object p) -> {
    	Foo foo = (Foo)p;
    	return new JsonPrimitive(foo.name + " via type adapter");
    }).toJson(new Foo("foo")));
    assertEquals("foo via type adapter", oson.des(Foo.class, (DataMapper p) -> {
    	String value = (String) p.getValueObject();
    	Foo foo = (Foo) p.getObj();
    	return new Foo (value + " via type adapter");
    }).fromJson("foo", Foo.class).name);
  }
  ...
  public void testStreamingFollowedByNonstreaming() {
    Gson gson = new GsonBuilder()
        ...
        .registerTypeAdapter(Foo.class, newDeserializer("deserializer"))
        .create();
    assertEquals("\"foo via serializer\"", oson.ser(Foo.class, (Object p) -> {
    	Foo foo = (Foo)p;
    	return new JsonPrimitive(foo.name + " via serializer");
    }).toJson(new Foo("foo")));
    assertEquals("foo via deserializer", oson.des(Foo.class, (DataMapper p) -> {
    	String value = (String) p.getValueObject();
    	Foo foo = (Foo) p.getObj();
    	return new Foo (value + " via deserializer");
    }).fromJson("foo", Foo.class).name);
  }
  ...
  public void testNonstreamingHierarchicalFollowedByNonstreaming() {
    Gson gson = new GsonBuilder()
        ...
        .create();
    assertEquals("\"foo via non hierarchical\"", oson.ser(Foo.class, (Object p) -> {
    	Foo foo = (Foo)p;
    	return new JsonPrimitive(foo.name + " via non hierarchical");
    }).toJson(new Foo("foo")));
    assertEquals("foo via non hierarchical", oson.des(Foo.class, (DataMapper p) -> {
    	String value = (String) p.getValueObject();
    	Foo foo = (Foo) p.getObj();
    	return new Foo (value + " via non hierarchical");
    }).fromJson("foo", Foo.class).name);
  }
