 ...
 */
public final class TypeHierarchyAdapterTest extends TestCaseBase {
  ...
  public void testTypeHierarchy() {
    ...
    //.pretty()
    String json = oson.toJson(company, Company.class);
    // different ordering
//    assertEquals("{\n" +
//        "  \"ceo\": {\n" +
//        "    \"userid\": \"eric\",\n" +
...
//        "}", json);
    ...
    Company copied = oson.fromJson(json, Company.class);
    //assertEquals(json, oson.toJson(copied, Company.class));
...
//        ((Manager) company.ceo.minions[2]).minions[1].userid);
  ...
  public void testRegisterSuperTypeFirst() {
    ...
    String json = oson.ser(Manager.class, (Object p) -> {
    	Manager m = (Manager)p;
    	return m.userid;
    }).setDefaultType(JSON_INCLUDE.NON_DEFAULT).toJson(manager, Manager.class);
    assertEquals("\"inder\"", json);
    Manager copied = oson.des(Manager.class, (DataMapper p) -> {
    	String value = (String) p.getValueObject();
    	Manager m = (Manager) p.getObj();
    	m.userid = StringUtil.unquote(value);
    	return m;
    }).fromJson(json, Manager.class);
